 ...
 * <p/>
 * To print "hello ansi world" in bold with blue foreground and white background: <pre
 * class="prettyprint"> Ansi ansi = new Ansi(Ansi.Attribute.BRIGHT, Ansi.Color.BLUE,
 * Ansi.Color.WHITE); ansi.{@link #out(String) out}("hello ansi world") </pre>
 * <p/>
 * same can be done as below: <pre class="prettyprint"> String msg = ansi.{@link #colorize(String)
 * colorize}("hello ansi world"); // msg is original string wrapped with ansi control sequences
 * System.out.println(msg); </pre>
 * <p/>
 * <b>Ansi Support:</b>
 * <p/>
 * Ansi might not be supported on all systems. Ansi is mostly supported by all unix operating
 * systems. <br><br> {@link Ansi#SUPPORTED} is a final boolean, that can be used to check whether
 * your console supports Ansi format; <br><br> Ansi class uses simple checks to decide whether ansi
 * is supported or not. Sometimes it may do wrong guess. In such cases you can override its decision
 * using following system property: <code>-DAnsi=true</code> or <code>-DAnsi=false</code> <br><br>
 * if {@link Ansi#SUPPORTED} is false, any ansi method will not produce ansi control sequences. so
 * you can safely use: <code>ansi.out("hello ansi world")</code> irrespective of ansi is supported
 * or not. if ansi is not supported, this will simply do <code>System.out.print("hello ansi
 * world")</code>
 *
...
public class Ansi {
    /**
     * specifies whether ansi is supported or not. <p><br> when this is false, it doesn't colorize
     * given strings, rather than simply returns the given strings <p><br> It tries best effort to
     * guess whether ansi is supported or not. But you can override this value using system property
     * "Ansi" (-DAnsi=true/false)
     */
    public static final boolean SUPPORTED = Boolean.getBoolean("Ansi") || System.console() != null;
     ...
     * </pre>
     * i.e, semicolon(;) separated values, where tokens are attribute, foreground and background
     * respectively.<br> if any non-trailing token in value is null, you still need to specify empty
     * value. for example:
     * <pre>
    ...
    /**
     * The string representation of this object. This string will be the same that is expected by
     * {@link #Ansi(String)}
     *
     ...
     * <p/>
     * if {@link #SUPPORTED} is false, it prints raw {@code message} to {@code ps} followed by
     * newline.
     *
     * @param ps      stream to print
     * @param message message to be colorized
